...
class TestModifyTasksAtomicity:
    ...
    def test_delete_task_removes_only_terminal_records(self, monkeypatch, tmp_path):
        tasks = [
            {'id': 'T-DONE', 'title': '已完成', 'state': 'Done'},
            {'id': 'T-ACTIVE', 'title': '进行中', 'state': 'Doing'},
        ]
        srv, _, tasks_path = _setup_server(monkeypatch, tmp_path, tasks)

        result = srv.handle_delete_task('T-DONE')
        assert result['ok'] is True
        assert [task['id'] for task in json.loads(tasks_path.read_text(encoding='utf-8'))] == ['T-ACTIVE']

        blocked = srv.handle_delete_task('T-ACTIVE')
        assert blocked['ok'] is False
        assert '完成或取消' in blocked['error']
        assert json.loads(tasks_path.read_text(encoding='utf-8'))[0]['id'] == 'T-ACTIVE'
