...
export default function CourtDiscussion() {
  ...
  // 皇帝发言
  const [history, setHistory] = useState<Array<{ session_id: string; topic: string; phase: string }>>([]);
  ...
  useEffect(() => { api.courtDiscussSessions().then(setHistory).catch(() => toast('朝议历史读取失败，请稍后重试', 'err')); }, [phase, toast]);
  ...
  // ── 推进讨论 ──
  const handleAdvance = useCallback(async (userMsg?: string, decree?: string, files: ChatAttachment[] = []) => {
    if (!session || loading || session.phase === 'concluded') return false;
    setLoading(true);

    try {
      const res = await api.courtDiscussAdvance(session.session_id, userMsg, decree, files.map((file) => file.id));
      ...
      setSession((prev) => {
        ...
        return {
          ...
          messages: res.messages || [...prev.messages, ...newMsgs, ...aiMsgs],
        };
      });
      ...
      setAutoPlay(false);
      toast(error instanceof Error ? error.message : '回奏失败，草稿已保留', 'err');
      return false;
    } finally {
      setLoading(false);
    }
  ...
  const handleDeleteSession = async (item: { session_id: string; topic: string; phase: string }) => {
    if (item.phase !== 'concluded') {
      toast('请先散朝，再删除朝议记录', 'err');
      return;
    }
    if (!window.confirm(`确认永久删除这场朝议记录？\n\n${item.topic}\n\n会话消息和附件都会删除，且无法恢复。`)) return;
    try {
      const result = await api.courtDiscussDelete(item.session_id);
      if (!result.ok) throw new Error(result.error || '删除失败');
      setHistory((current) => current.filter((entry) => entry.session_id !== item.session_id));
      if (session?.session_id === item.session_id) handleReset();
      toast('朝议记录已删除', 'ok');
    } catch (error) {
      toast(error instanceof Error ? error.message : '删除失败', 'err');
    }
  };
  ...
  if (phase === 'setup') {
    return (
      <div className="space-y-6">
        ...
        {history.length > 0 && <section className="court-history"><h3>朝议记录</h3>{history.map((item) => <div className="court-history-entry" key={item.session_id}><button className="btn btn-g" onClick={async () => {
          try {
            const result = await api.courtDiscussSession(item.session_id);
            if (!result.ok) throw new Error(result.error || '读取失败');
            setSession(result as unknown as CourtSession); setPhase('session'); setSimulated(false);
          } catch { toast('朝议记录读取失败', 'err'); }
        }}>{item.topic} · {item.phase === 'concluded' ? '已结束' : '议事中'}</button><button className="btn btn-g court-history-delete" onClick={() => void handleDeleteSession(item)} disabled={item.phase !== 'concluded'} title={item.phase === 'concluded' ? '永久删除记录' : '请先散朝'}>🗑 删除</button></div>)}</section>}
      </div>
    );
  }
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2 bg-[var(--panel)] rounded-xl px-4 py-2 border border-[var(--line)]">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button
            onClick={handleDice}
            disabled={diceRolling || loading || session?.phase === 'concluded'}
      ...
      {simulated && <div className="yushu-alert" role="status">当前为规则模拟回复，未调用模型分析。请检查供应商配置。</div>}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-3">
        ...
        {/* 右侧：聊天记录 */}
        <div className="bg-[var(--panel)] rounded-xl border border-[var(--line)] flex flex-col min-w-0">
          {/* 消息列表 */}
          <div className="flex-1 overflow-y-auto p-3 space-y-2" style={{ minHeight: 200, maxHeight: 500 }}>
            {messages.map((msg, i) => (
              <MessageBubble key={i} msg={msg} officials={officials} scope={`court-${session?.session_id}`} />
            ))}
            {loading && (
          ...
          {session?.phase !== 'concluded' && (
            <div>
              <ChatComposer key={session?.session_id} scope={`court-${session?.session_id}`} label="朝堂发言" busy={loading} placeholder="朕有话说…" sendLabel="发言" onSend={(text, files) => handleAdvance(text, undefined, files)} />
              <button
          ...
          {session?.phase === 'concluded' && (
            <button
              onClick={() => void handleDeleteSession({ session_id: session.session_id, topic: session.topic, phase: session.phase })}
              disabled={loading}
              className="px-3 py-1.5 rounded-lg text-xs border border-red-700/40 text-red-400 hover:bg-red-900/20 transition"
            >
              🗑 删除记录
            </button>
          )}
    ...
    return (
      <div className="flex justify-end">
        <div className="max-w-[80%] bg-gradient-to-br from-amber-900/40 to-amber-800/20 rounded-xl px-3 py-2 border border-amber-700/30">
          ...
          <AttachmentList scope={scope} files={msg.attachments} />
        </div>
      </div>
    );
  }
