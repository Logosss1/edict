...
桌面迁移工作区不保留视频、GIF 和截图媒体；请直接运行 `desktop` 中的构建命令体验完整看板。
...
截图媒体不随桌面迁移工作区保留；功能清单和自动化验证记录位于项目根目录的 `specs/`。
...
> 💡 **看板即开即用**：`server.py` 提供预构建的 React 前端 `dashboard/dist/`
