...
contextBridge.exposeInMainWorld('edictDesktop', {
  ...
  chooseWorkspace: (mode: 'create' | 'existing' = 'create') => ipcRenderer.invoke('workspace:choose', { mode }),
  useWorkspacePath: (path: string) => ipcRenderer.invoke('workspace:use-path', { path }),
  ...
  useWorkspaceAsProject: () => ipcRenderer.invoke('workspace:use-root-project'),
  ...
  probeGateway: () => ipcRenderer.invoke('openclaw:gateway-probe'),
  checkWorkspaceAccess: (path?: string) => ipcRenderer.invoke('workspace:preflight', { path }),
  openWorkspacePermissions: () => ipcRenderer.invoke('workspace:open-permissions'),
