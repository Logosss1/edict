...
export default function SessionsPanel() {
  ...
  const deleteRecord = async (task: Task) => {
    if (!['Done', 'Cancelled'].includes(task.state)) return;
    if (!confirm(`确认永久删除任务 ${task.id}？\n\n${humanTitle(task, labelMap)}\n\n记录会从所有任务页面移除，且无法恢复。`)) return;
    try {
      const result = await api.deleteTask(task.id);
      if (!result.ok) throw new Error(result.error || '删除失败');
      setDetailTask(null);
      toast(result.message || '任务记录已删除', 'ok');
      loadAll();
    } catch (error) {
  ...
  return (
    <div>
      ...
      {detailTask && (
        <SessionDetailModal task={detailTask} labelMap={labelMap} emojiMap={emojiMap} onClose={() => setDetailTask(null)} onDelete={() => void deleteRecord(detailTask)} />
      )}
    </div>
  );
}
    ...
    <div className="modal-bg open" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          {['Done', 'Cancelled'].includes(st) && (
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 16 }}>
              <button className="btn btn-danger" onClick={onDelete} style={{ fontSize: 12, padding: '6px 16px' }}>
                🗑 删除记录
              </button>
            </div>
          )}
