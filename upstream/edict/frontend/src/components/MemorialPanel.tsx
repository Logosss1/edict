...
export default function MemorialPanel() {
  ...
  const deleteRecord = async (task: Task) => {
    ...
    if (!confirm(`确认永久删除这份奏折？\n\n${task.title || task.id}\n\n对应任务记录也会一并移除，且无法恢复。`)) return;
    try {
      const result = await api.deleteTask(task.id);
      if (!result.ok) throw new Error(result.error || '删除失败');
      setDetailTask(null);
      toast(result.message || '奏折已删除', 'ok');
      loadAll();
    } catch (error) {
  ...
  return (
    <div>
      ...
      {/* Detail Modal */}
      {detailTask && (
        <MemorialDetailModal task={detailTask} onClose={() => setDetailTask(null)} onExport={exportMemorial} onDelete={() => void deleteRecord(detailTask)} />
      )}
    </div>
  );
}
